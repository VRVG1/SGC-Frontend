...
const ReportesCheck = (props) => {
  ...
  const getGenerann = useCallback(async () => {
    await getGeneran(auth.user.token)
      .then((res) => {
        setGeneran(res);
      })
      .catch((err) => {
  ...
  const getAsignan = useCallback(async () => {
    await getAsignanAllUser(auth.user.token)
      .then((res) => {
        setAsignan(res);
      })
      .catch((err) => {
  ...
  const getMaestros = useCallback(async () => {
    getAllUsuarios(auth.user.token)
      .then((res) => {
        setMaestros(res);
  ...
  const getReportes = useCallback(async () => {
    await getAllReportes(auth.user.token)
      .then((res) => {
        setReportes(res);
      })
      .catch((err) => {
  ...
  const getMaterias = useCallback(async () => {
    await getAllMaterias(auth.user.token)
      .then((res) => {
        setMaterias(res);
      })
      .catch((err) => {
  ...
  const getPDFName = useCallback(async (PK) => {
    await getPDFNames(PK, auth.user.token)
      .then((res) => {
        setPdfNames(res);
      })
      .catch((err) => {
    ...
    if (name === "nombreMasters") {
      ...
      if (value.length > 0) {
        filteredMaestros = maestros.filter((maestro) => {
          return maestro.Nombre_Usuario.toLowerCase().includes(
            value.toLowerCase()
          );
        });
      } else {
      ...
      if (value.length > 0) {
        filteredMaterias = materias.filter((materia) => {
          return materia.Nombre_Materia.toLowerCase().includes(
            value.toLowerCase()
          );
        });
      } else {
      ...
      {loading === false ? (
        <>
            ...
            <div className="ReportesCheck-search">
              ...
              <form>
                <div className={hidden.hidden1}>
                  ...
                  <span
                    className={"highlight reportes-check" + oculto.hidden1}
                  ></span>
                  <span
                    className={"bottomBar reportes-check" + oculto.hidden1}
                  ></span>
                  <label className={"reportes-check" + oculto.hidden1}>
                    Nombre del Docente
                  </label>
                </div>
                {/* Carrera */}
                ...
                <div className={hidden.hidden4}>
                  ...
                  <span
                    className={"highlight reportes-check" + oculto.hidden4}
                  ></span>
                  <span
                    className={"bottomBar reportes-check" + oculto.hidden4}
                  ></span>
                  <label className={"reportes-check" + oculto.hidden4}>
                                  ...
                                  <table className="table-ReportesCheck">
                                    ...
                                    <tbody className="tbody-ReportesCheck">
                                      {asignan.map((asignan2, index) => {
                                        ...
                                        if (
                                          PK?.ID_Generacion === null ||
                                          PK?.ID_Generacion === undefined
                                        ) {
                                          PK = -1;
                                        } else {
                                          PK = PK.ID_Generacion;
                                        ...
                                        if (
                                          asignan2.ID_Usuario === maestro.PK
                                        ) {
                                          let si = materias.filter(
                                            (materia) =>
                                              materia.pik ===
                                              asignan2.ID_Materia
                                          )[0].Nombre_Materia;
                                          ...
                                          if (
                                            typeof dataInput.nombreMateria ===
                                            "undefined"
                                          ) {
                                            return (
                                              <tr key={index}>
                                            ...
                                            if (
                                              si
                                                .toLowerCase()
                                                .includes(
                                                  dataInput.nombreMateria
                                                ) ||
                                              dataInput.nombreMateria === si
                                            ) {
                                              return (
                                                <tr key={index}>
                                                  <td>{si}</td>
          ...
          <Modal
            ...
            setShow={setShowModalArchivos}
            title={reporteMaster.reporte + " - " + reporteMaster.maestro}
          >

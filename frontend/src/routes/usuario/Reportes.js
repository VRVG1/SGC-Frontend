      ...
      for (let i = 0; i < reportesFiltrados.length; i++) {
        let diff =
          (new Date(
            reporteName.filter(
              (reporte) =>
                reporte.ID_Reporte === reportesFiltrados[i].ID_Reporte
            )[0].Fecha_Entrega
          ).getTime() -
            si) /
          (1000 * 60 * 60 * 24)
          ...
          {Object.keys(reportes).length !== 0 ? (
            <>
              <div className="reportesUser-Main">
                <div className="reportesUser-Container">
                  <div className="listReportes">
                    <ul>
                      {Object.keys(reporteName).length !== 0 ? (
                        reporteName.map((reporte, index) => {
                          if (reporte.Opcional) {
                            return (
                              <li key={index}>
                                <div
                    ...
                    {selMateria.index !== null ? (
                      <>
                        <div className="subirArchivos__module">
                          ...
                          <button onClick={fileSummit}>Enviar</button>
                        </div>
              ...
              <Modal
                ...
                setShow={setShowModalBorrar}
                title={"Eliminar Archivo"}
              >
                <div className="modal-borrar">
                  <p className="alertMSM">
                    Estas segurode de borrar toda la entrega:
                  </p>
                  <MostrarArchivos />
